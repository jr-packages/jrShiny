...
# do this in advance so we don't download the
# data every time
tmp = tempdir()
url = "http://personal.tcu.edu/kylewalker/data/mexico.zip" # take some mexico shapefile data
file = basename(url)
...
mexico = {
  ...
  on.exit({
    unlink(tmp);unlink(file)
    }
    )
  readOGR(dsn = tmp, layer = "mexico", encoding = "UTF-8") #OGR vector map to spatial object
}

# mexico crime time series
...
server = function(input, output, session) {
  ...
  output$dygraph1 = renderDygraph({
    ...
    crime_wide = reshape(
      crime_mexico$hd[, c("date", "rate", "state_code"), drop = FALSE],
      v.names = "rate",
      idvar = "date",
      timevar = "state_code",
      direction = "wide"
  ...
  observeEvent(input$map1_shape_click, {
    ...
    output$dygraph1 = renderDygraph({
      dygraph(
        xts::as.xts(state_crime_data[, "rate", drop = FALSE]),
        ylab = paste0(
          "homicide rate ",
