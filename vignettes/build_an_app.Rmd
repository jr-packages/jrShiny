...
4. The data table looks a bit rubbish because it's going off the end of the table. Make it only displayed the title, year, duration, classification and rating columns.
...
7. Using either `observeEvent()` or `observe()`, filter `rvs$data` accordingly everytime the user selects a new classification of movie. You can filter the data set like so 
```{r, eval = FALSE}
movies_new[movies_new$classification == input$id_of_widget,]
```
    
8. Add the argument `multiple = TRUE` to your `selectInput()` and change the `==` symbol in your filter command to `%in%`. What happens?
